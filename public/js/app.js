...
async function initLiff() {
  ...
  try {
    ...
    if (liff.isLoggedIn()) state.liffProfile = await liff.getProfile();
  } catch (err) {
...
function renderQuickList() {
  ...
  $('quickList').innerHTML = quick.length ? quick.map(productCardSmall).join('') : '<div class="empty">尚未設定快速補貨商品</div>';
}

function renderCategories() {
  const categories = ['全部', ...new Set(state.products.map(p => p.category).filter(Boolean))];
  $('categoryList').innerHTML = categories.map(c => `<button class="category-chip ${c === state.category ? 'active' : ''}" data-category="${c}">${c}</button>`).join('');
}
...
function renderProducts() {
  ...
  $('productGrid').innerHTML = state.filtered.length ? state.filtered.map(productCard).join('') : '<div class="empty">找不到商品，可以直接傳訊息給客服。</div>';
}
...
function renderCart() {
  ...
  $('cartItems').innerHTML = items.length ? items.map(item => `<div class="cart-item">
  ...
  </div>`).join('') : '<div class="empty">購物車目前是空的</div>';
  $('cartTotal').textContent = money(items.reduce((sum, item) => sum + item.subtotal, 0));
}

function renderCheckout() {
  const items = getCartItems();
  $('checkoutItems').innerHTML = items.map(item => `<div class="summary-line"><span>${item.name} × ${item.qty}</span><span>${money(item.subtotal)}</span></div>`).join('');
...
function bindEvents() {
  $('searchInput').addEventListener('input', e => { state.keyword = e.target.value; applyFilter(); });
  $('reloadBtn').addEventListener('click', loadProducts);
  $('cartBtn').addEventListener('click', () => { renderCart(); $('cartDrawer').classList.remove('hidden'); });
  ...
  document.body.addEventListener('click', e => {
    ...
    if (category) { state.category = category; renderCategories(); applyFilter(); }

    const close = e.target.dataset.close;
    if (close === 'cart') $('cartDrawer').classList.add('hidden');
